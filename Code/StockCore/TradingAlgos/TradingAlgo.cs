...
namespace StockTrader.API.TradingAlgos
{
    public abstract partial class TradingAlgo : ITradingAlgo
    {
        ...
        // Diagnostics
        protected double _minPriceBeforeSqrOff = 0;
        protected double _maxPriceBeforeSqrOff = 0;
        ...
        protected bool _isGivenMktClosingMsg = false;

        // Trade Summary
        public double LTP = 0;
        protected double _lastSqrOffTradeProfit = 0;
        protected double _lastSqrOffTradePrice = 0;
        protected DateTime _lastSqrOffTradeTime;
        ...
        private bool _isOrderExecutionPending;
        public bool IsOrderExecutionPending
        {
            get
            {
                return S != null ? S.IsOrderExecutionPending : _isOrderExecutionPending;
            }
            set
            {
                if (S != null)
                    S.IsOrderExecutionPending = value;
                _isOrderExecutionPending = value;
            }
        }
        ...
        public double GetSquareOffProfitPerc()
        {
            ...
            try
            {
                ...
                double profitPoints = orderPrice - GetPairedSquareOffOrder(isBuy ? Position.BUY : Position.SELL, false).OrderPrice; // if sqroff done outside then sometimes this may lead to wrong state
        ...
        {
            double profitAmt = Math.Round((GetSquareOffProfitPerc() * AlgoParams.I.Qty * S.Ticks.CurrTick.Di.LastTradedPriceDouble) / 100, 2);
            return profitAmt;
        }
        public virtual StockOrder GetOpenPosition()
        {
        ...
        protected void PostProcAddTick()
        {
            try
            {
                if (!AlgoParams.IsReplayMode)
                {
                    ...
                    //    if (sendAlert)
                    //    {
                    //        double profitAmt = Math.Round((profitPerc * AlgoParams.I.Qty * S.Ticks.CurrTick.Di.LastTradedPriceDouble) / 100, 2);
                    ...
                    //            pnl, S.Ticks.CurrTick.Di.LastTradedPriceDouble);
        ...
        public virtual BrokerErrorCode Prolog()
        {
        ...
        public virtual BrokerErrorCode Epilog()
        {
        ...
        {
            S.Ticks.AddTick(new DTick()
            {
                Di = si.D.Q,
                TickNumber = S.TotalTickCount
            });

            // update diagnostics min max between trades
            if (S.Ticks.CurrTick.Di.LastTradedPriceDouble > _maxPriceBeforeSqrOff)
                _maxPriceBeforeSqrOff = S.Ticks.CurrTick.Di.LastTradedPriceDouble;
            else if (S.Ticks.CurrTick.Di.LastTradedPriceDouble < _minPriceBeforeSqrOff)
                _minPriceBeforeSqrOff = S.Ticks.CurrTick.Di.LastTradedPriceDouble;

            S.IsOrderExecutionPending = false;

            S.IsMarketClosing = MarketUtils.IsTimeAfter320(S.Ticks.CurrTick.Di.UpdateTime);

            // Trace mkt closing msg for debug purposes
            if (S.IsMarketClosing && !_isGivenMktClosingMsg)
            {
                FileTracing.TraceOut("Market closing set");
                _isGivenMktClosingMsg = true;
            }

            if (!AlgoParams.IsMock && !MarketUtils.IsTimeAfter915(S.Ticks.CurrTick.Di.QuoteTime))  // start only when the day live quote appears
                return false;

            if (AlgoParams.AllowInitialTickStabilization && !MarketUtils.IsTimeAfter920(S.Ticks.CurrTick.Di.UpdateTime))  // initial 5 minute no-trade buffer for the prices to settle down
                return false;

            // Stats
            double ltp = GetPriceForAnalysis(S.Ticks.CurrTick.Di);
            mAverageTradePrice += ltp;
            TimeOfLastTickOfPeriod = S.Ticks.CurrTick.Di.QuoteTime;
            ...
            //    S.IsNextDay = (S.Ticks.CurrTick.Di.QuoteTime.Date != S.DayToday.Date);
            ...
            //S.Ticks.PrevTick = S.Ticks.CurrTick;
            ...
            TimeOfLastTickOfDay = S.Ticks.CurrTick.Di.QuoteTime;
            ...
            , bool isMarketOrder = false, bool passThroughOrder = false)
        {
            ...
            var newOrder = new StockOrder(orderDirection, 0, S.Ticks.CurrTick);
            ...
            if (isOrderExecuted)
            {
                ...
                {
                    _minPriceBeforeSqrOff = _maxPriceBeforeSqrOff = newOrder.OrderPrice;
        ...
        protected double AfterSquareOff(StockOrder newOrder, StockOrder squaredOffOrder)
        {
            ...
            newOrder.OrderProfit = nettProfitAmt;

            _lastSqrOffTradePrice = newOrder.OrderPrice;
        ...
        protected void DoFirstTickInititalizations()
        {
            ...
            if (!S.IsFirstAlgoTickSeen)
            {
                ...
                TimeOfFirstTickOfPeriod = S.Ticks.CurrTick.Di.QuoteTime;
                TimeOfFirstTickOfDay = S.Ticks.CurrTick.Di.QuoteTime;
                S.DayToday = S.Ticks.CurrTick.Di.QuoteTime;
        ...
        protected bool DoEODWindup()
        {
            ...
            if (doesNeedEODWindup)
            {
                ...
                if (numOpenPositions != 0)
                {
                    ...
                    OrderPlaceAlgoCode closingOrderCode = AlgoTryPlaceOrder(isBuy, S.Ticks.CurrTick, false, placeMarketOrder, false);

                    if (closingOrderCode == OrderPlaceAlgoCode.RETRY)
                        S.IsOrderExecutionPending = true;
        ...
        protected void DoEODStatsWindup()
        {
            FileTracing.TraceOut("Going to do EOD stats windup");
            ...
            _eodTradeStats.roi_percentage = (_eodTradeStats.expected_profit / (S.Ticks.PrevTick.Di.LastTradedPriceDouble * AlgoParams.I.Qty)) *
                                            100 * (1 / AlgoParams.MarginFraction);
            _eodTradeStats.actual_roi_percentage = (_eodTradeStats.actual_profit / (S.Ticks.PrevTick.Di.LastTradedPriceDouble * AlgoParams.I.Qty)) *
                                                  100 * (1 / AlgoParams.MarginFraction);
            ...
            //S.Ticks.MinTick = S.Ticks.MaxTick = null;
            _lastPeak = null;
        ...
        protected void SetValuesForNextDay()
        {
            ...
            if (S.IsNextDay && S.IsEODWindupDone)
            {
                ...
                _eodTradeStats = new EODTradeStats(AlgoParams.R1, AlgoParams.R2);
                _eodTradeStats.max_price = _eodTradeStats.min_price = GetPriceForAnalysis(S.Ticks.CurrTick.Di);


                S.Ticks.ReInit(AlgoParams.IsConsiderPrevClosing);

                //if (AlgoParams.IsConsiderPrevClosing)
                //{
                //    S.Ticks.MinTick = S.Ticks.MaxTick = S.Ticks.PrevTick;
                //}
                //else
                //{
                //    if (S.Ticks.MinTick == null || S.Ticks.MaxTick == null)
        ...
        protected bool PlaceOrder(StockOrder orderToPlace, DerivativeSymbolQuote di, bool doNoPriceCheck = true, bool isMarketOrder = false)
        {
            ...
            if (!isMarketOrder && !doNoPriceCheck)
            {
                ...
                if (diff < 0 && Math.Abs(diffPerc) > 0.2)
                {
        ...
        protected virtual void AlgoStateMgmtAfterRemoteCommand(bool isSquareoff) { }
        ...
        protected virtual void UpdatePositionsOnReset()
        {
            if (S.TotalBuyTrades != S.TotalSellTrades)
            {
                ...
                var newOrder = new StockOrder(pos, S.Ticks.CurrTick.Di.LastTradedPriceDouble, S.Ticks.CurrTick);
        ...
        protected OrderPlaceAlgoCode TryImmediateSquareOff(bool isContinueTillSuccess, bool placeMarketOrder, bool passThroughOrder)
        {
            ...
            while (numOpenPositions != 0)
            {
                ...
                orderPlaceCode = AlgoTryPlaceOrder(isBuy, S.Ticks.CurrTick, true, placeMarketOrder, passThroughOrder); // Place market order
                ...
                if (orderPlaceCode == OrderPlaceAlgoCode.SUCCESS)
                {
                    ...
                    S.Ticks.UpdateMinMax();
                    S.PercChangeThreshold = AlgoParams.PercMarketDirectionChange;

                    _allPeaks.Add(S.TotalTickCount, _lastPeak);

                    AlgoStateMgmtAfterRemoteCommand(true);
                }
                else if (orderPlaceCode != OrderPlaceAlgoCode.RETRY)
                    break;
        ...
        public void SquareOffAtProfitReset(bool doAlert = true)
        {
            var isOpenPos = SquareOffAtProfit(false);
            ...
            var status = isOpenPos ? "Set" : " No open positions to SquareOff";
            string desc = string.Format("Algo {0}: SquaredOffAtProfit, Status:{1} and ResetCore", AlgoParams.ToString(), status);
            OnRemoteCmd("SquareOffAtProfitReset", desc, doAlert);
        }
        ...
        public AlgoRunStats GetEOPStats(double ltp)
        {
            ...
            plStats.Profit = S.TotalActualNettProfitAmt;
            ...
            double nettDay = S.TotalActualNettProfitAmt + nettOpenPositionAmt;
        ...
        public void GetEOPStatsAndTrace(double ltp)
        {
            ...
            sb.AppendLine("Total Trade Diff. amount (after brokerage) : " + S.TotalActualNettProfitAmt);
            sb.AppendLine("Total Trade Diff. amount (after brokerage) %: " + (S.TotalActualNettProfitAmt / (ltp * AlgoParams.I.Qty)) * 100);

            double TotalProfitWithoutBrokerage = S.TotalActualNettProfitAmt + S.TotalBrokerageAmt;
            ...
            _eopTradeStats.actual_profit = S.TotalActualNettProfitAmt;
            _eopTradeStats.brokerage = S.TotalBrokerageAmt;
            _eopTradeStats.quantity = AlgoParams.I.Qty;
            _eopTradeStats.actual_roi_percentage = ((S.TotalActualNettProfitAmt * 100) / ((mAverageTradePrice == 0 ? 1 : mAverageTradePrice) * AlgoParams.I.Qty)) * (1 / AlgoParams.MarginFraction);
