    ...
    public class UpstoxAverageTheBuyThenSell : UpstoxBuySellBase
    {
        ...
        private void PlaceBuyOrderIfEligible()
        {
                ...
                && todayOutstandingQty < maxTodayOutstandingQtyAllowed)
            {
                ...
                var qtyAgressionFactor = 0.5;//get from config
                ...
                totalAvgHoldingPrice = Math.Round(totalAvgHoldingPrice, 2);

                var priceToCompare = Math.Max(totalAvgHoldingPricelastBuyPrice > 0 ? lastBuyPrice : 0;


                var priceDiffPct = (totalAvgHoldingPrice - calculatedToBuyPrice) / totalAvgHoldingPrice;
                var priceDiffMultiple = priceDiffPct / 0.05; // 5% buckets
                var priceBucketfactor = priceBucketFactorForQty[(int)priceDiffMultiple];
                priceAgressionFactor = Math.Max(1, priceDiffMultiple * priceBucketfactor);

                qtyStrategy = string.Format("totalAvgHoldingPrice {0}, totalOutstandingQty {1}, priceDiffMultiple {2} priceBucketfactor {3} priceAgressionFactor {4} qtyAgressionFactor{5}",
                    totalAvgHoldingPrice, totalOutstandingQty, Math.Round(priceDiffMultiple, 2), priceBucketfactor, Math.Round(priceAgressionFactor, 4), qtyAgressionFactor);
            }
            else
            {
                qtyStrategy = string.Format("No Holdings, so priceAgressionFactor is {0}",
totalAvgHoldingPrice, totalOutstandingQty, Math.Round(priceDiffMultiple, 2), priceBucketfactor, Math.Round(priceAgressionFactor, 4));

            }
            var qtyCurve = totalOutstandingMultiple * qtyAgressionFactor * priceAgressionFactor;
